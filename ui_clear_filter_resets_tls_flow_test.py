...
def main():
    ...
    # Ensure timeline has a Session row
    ...
    # Double-click the session's label column (this applies session filter + shows TLS Flow)
    model = view.model()
    ...
    # Now clear filter; expect TLS Flow reset to placeholder
    win.clear_command_family_filter()

    ok = wait_for(lambda: hasattr(win, "tls_tree") and win.tls_tree.topLevelItemCount() == 0, 3000)
    assert ok, "TLS Flow steps were not cleared"

    txt = ""
    try:
        txt = win.tls_summary_label.text()
    except Exception:
        txt = ""
    assert "select a session" in (txt or "").lower(), f"Unexpected TLS summary label after clear: {txt!r}"

    print("PASS: Clear Filter resets TLS Flow placeholders")
    return 0
